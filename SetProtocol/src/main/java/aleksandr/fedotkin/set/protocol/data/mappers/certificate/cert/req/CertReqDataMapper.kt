...
//        return CertReqData(
//            rrpID = bigIntegerMapper.map(value = model.rrpID),
//            lidEE = bigIntegerMapper.map(value = model.lidEE),
//            challEE3 = bigIntegerMapper.map(value = model.challEE3),
//            lidCA = bigIntegerMapper.map(value = model.lidCA),
//            challCA = bigIntegerMapper.map(value = model.challCA),
//            requestType = model.requestType,
//            requestDate = dateTimeMapper.map(dateTime = model.requestDate),
//            idData = idDataMapper.map(model = model.idData),
//            regFormID = bigIntegerMapper.map(value = model.regFormID),
