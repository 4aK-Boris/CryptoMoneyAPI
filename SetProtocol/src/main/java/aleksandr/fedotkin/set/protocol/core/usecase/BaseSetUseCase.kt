...
abstract class BaseSetUseCase<T : Model, R : DTO> : BaseUseCase<T, R> {

    private val messageWrapperRepository by inject<MessageWrapperRepository<T, R>> {
        parametersOf(
            repository.mapper
        )
    }
    ...
    suspend fun checkRRPID(rrpid: BigInteger) {
